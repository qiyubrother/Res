$(function () {
    ...
    $("#showNewDialog").click(function () {
        $("#dialogContainer").empty().load("/Admin/pop/NewSearchKey.aspx?_" + Math.random(), function () {
            $("#myModal").modal({
                backdrop: true,
                keyboard: true
            });
    ...
    $("a[name='showEditDialog']").click(function () {
        ...
        $("#dialogContainer").empty().load("/Admin/pop/EditSearchKey.aspx?id=" + id + "&_" + Math.random(), function () {
            $("#myModal").modal({
                backdrop: true,
                keyboard: true
            });
    ...
    $("#dialogContainer").on("click", "#newSave", function () {
        ...
        var SearchKeyText = $("#SearchKeyText").val();
        $.ajax({
            type: "post",
            datatype: "json",
            url: "/Admin/Action/ActionSearchKey.ashx?action=create" + "&SearchKeyId=" + SearchKeyId + "&SearchKeyText=" + SearchKeyText,
            success: function (d) {
    ...
    $("#dialogContainer").on("click", "#editSave", function () {
        ...
        $.ajax({
            type: "post",
            datatype: "json",
            url: "/Admin/Action/ActionSearchKey.ashx?action=update" + "&SearchKeyId=" + SearchKeyId + "&SearchKeyText=" + SearchKeyText,
            success: function (d) {
                var jsondata = JSON.parse(d);
                // 关闭对话框
                $("#editClose").click();
    ...
    $("#dialogContainer").on("click", "#delYes", function () {
        ...
        $.ajax({
            type: "post",
            datatype: "json",
            url: "/Admin/Action/ActionSearchKey.ashx?action=delete" + "&SearchKeyId=" + ids,
            success: function (d) {
                var jsondata = JSON.parse(d);
                // 关闭对话框
                $("#editClose").click();
